...
public class PlaylistReader {
     ...
     * 
     * @param songsFileName     file name of the program's songs.
     * 
     * @param playlistsFileName file name of the suggested playlists.
     * 
     * @throws java.text.ParseException      if an error has been reached while
     *                                       parsing the input file.
     * 
     * @throws DailyMixDataException         if data is incorrect in the input
     *                                       files.
     * 
     * @throws java.io.FileNotFoundException if the input file is inaccessible or
     *                                       does not exist.
     */
     ...
     * Playlist objects.
     * 
     * @param playlistFileName a playlist file name.
     * 
     * @throws ParseException        if an error has been reached while parsing the
     *                               input file. For example, percentages are not
     *                               valid
     * 
     * @throws DailyMixDataException if data is incorrect in the input files.
     * 
     * @throws FileNotFoundException if the input file is inaccessible or does not
     *                               exist.
    ...
    private Playlist[] readPlaylistFile(String playlistFileName)
        ...
        Playlist[] parsedPlaylists = new Playlist[PlaylistCalculator.NUM_PLAYLISTS];
        ...
        while (file.hasNextLine() && playlistCounter < PlaylistCalculator.NUM_PLAYLISTS) {
            ...
            if (tokens.length < 8) {
                ...
                throw new ParseException("insufficient tokens", playlistCounter);
            } else if (tokens.length > 8) {
                // 2b.TODO: throw corresponding exception, along with a message
                
                throw new ParseException("Exceeding tokens", playlistCounter);
            }
                ...
                Playlist newPlaylist = new Playlist(playlistName, 
                        minPop, minRock,minCountry, maxPop,
                        maxRock, maxCountry, maxSongsPerPlaylist);
        ...
        if (playlistCounter < PlaylistCalculator.NUM_PLAYLISTS) {
            ...
            throw new DailyMixDataException("insufficient" + PlaylistCalculator.NUM_PLAYLISTS);
        }
    ...
    // -------------------------------------------------------------------------
    /**
     * Parses data from a song file to populate and return a queue of Song objects.
     * 
     * @param songFileName a song file name.
     * 
    ...
    private ArrayQueue<Song> readQueueFile(String songFileName)
        ...
        ArrayQueue<Song> parsedSongs = new ArrayQueue<Song>(ArrayQueue.DEFAULT_CAPACITY);
        ...
        while (file.hasNextLine()) {
            ...
            if (tokens.length != SONG_TOKENS && tokens.length != SONG_TOKENS - 1) {
                // 1.TODO: throw corresponding exception, along with a message
                throw new ParseException("insufficient tokens" + read, 0);
            }
            ...
            if (!isInValidPercentRange(pop, rock, country)) {
                throw new DailyMixDataException("Invalid" + songName);
            }
            ...
            // declare and instantiate a new Song accordingly.
                Song song ;
                if (playlist.isEmpty()) {
                    song = new Song(songName, pop, rock, country, "");
                }
                else {
                    song = new Song(songName, pop, rock, country, playlist);
                }
     ...
     * possible values for a genre's percent composition N.B. use the "public static
     * final variables" in PlaylistCalculator to avoid hard coding
     * 
     * @param num1 first genre percent composition (pop)
     * 
     * @param num2 second genre percent composition (Rock)
     * 
     * @param num3 third genre percent composition (Country)
     * 
    ...
    private boolean isInValidPercentRange(int num1, int num2, int num3) {
        ...
        if (num1 >= PlaylistCalculator.MIN_PERCENT && num1 <= PlaylistCalculator.MAX_PERCENT &&
